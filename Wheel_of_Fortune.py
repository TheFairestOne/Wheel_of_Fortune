...
# General player class definition
class WOFPlayer():
    def __init__(self, name, money = 0, prizes = []):
        self.name = name
...
class WOFHumanPlayer(WOFPlayer):
    def getMove(self, category, obscuredPhrase, guessed):
        base_prompt = self.__str__()
        base_prompt += """
Category: {}
Phrase:   {}
Guessed:  {}""".format(category, obscuredPhrase, ', '.join(sorted(guessed)))
        base_prompt += "Guess a letter, phrase, or type 'exit' or 'pass':"
        while True:
            userinp = input(base_prompt)
            move = str(userinp)
            if move == 'pass':
                return
            elif move == 'exit':
...
# Computer player class definition
class WOFComputerPlayer(WOFPlayer):
    ...
    def __init__(self, name, diff):
        ...
        self.difficulty = diff        #random.randint(0,10)
    ...
    def getPossibleLetters(self, guessed):
        possible_lst = []
        possible_lst = [x for x in LETTERS if x not in guessed]
        if self.prizeMoney < VOWEL_COST:
            possible_lst = [x for x in possible_lst if x not in VOWELS]
        return possible_lst
    ...
    def getMove(self, category, obscuredPhrase, guessed):
        ...
        if len(letters) == 0:
            ...
            moves = [x for x in self.SORTED_FREQUENCIES if x in letters]
            good_move = self.smartCoinFlip()
            if good_move == True:
                move = moves[-1]
            elif good_move == False:
                move = random.choice(letters)
        return move
...
while True:
    ...
    # Move on to the next player (or go back to player[0] if reached the end)
